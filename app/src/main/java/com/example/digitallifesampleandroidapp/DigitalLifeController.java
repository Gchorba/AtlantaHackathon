...
public class DigitalLifeController {
			...
			protected JSONObject doInBackground(JSONObject... params) {
				...
				try {
					data = getResource("/penguin/api/authtokens",
							...
							new BasicNameValuePair("appKey", "VE_6485B0A98964AECA_1"),
							new BasicNameValuePair("userId", "553474466"),
							new BasicNameValuePair("password", "NO-PASSWD"));
    ...
    public String updateAlarm() {
        Runnable r = new Runnable() {
            ...
            public void run() {
                try {
                    HttpPost request = new HttpPost(baseURL + "/penguin/api/"+gatewayGUID+"/alarm");
                    System.out.println("UpdateAlarm:  "+ request.getURI());

                    request.addHeader("authToken", authToken);
                    request.addHeader("requestToken", requestToken);
                    request.addHeader("appKey", penguinAppId);
                       JSONObject object1 = new JSONObject();
                    object1.put("status", "PoliceRemote");
                    object1.put("bypass", "");
String message = object1.toString();
